...
function Tambah(e, sheet){
  ...
  for(var i=1;i<=lr;i++){
    ...
    if(id1==idruta){
      flag=0;
      sheet.getRange(i,2).setValue(idnks);
      sheet.getRange(i,3).setValue(nus);
      sheet.getRange(i,4).setValue(hasil);
      sheet.getRange(i,5).setValue(art);
      sheet.getRange(i,6).setValue(catatan);
 ...
 // Logger.log(flag);
  ...
  if(flag==1){
  var rowData = sheet.appendRow([idruta,idnks,nus,hasil,art,catatan,alamat,koordinat,tgl,new Date]);  
