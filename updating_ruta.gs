...
function Tambah(e, sheet){
  ...
  var jksebelum = e.parameter.jksebelum;
  var jkhasil = e.parameter.jkhasil;
  var rutahasil = e.parameter.rutahasil;
  ...
  for(var i=1;i<=lr;i++){
    ...
    if(id1==idnks){
      flag=0;
      sheet.getRange(i,2).setValue(jksebelum);
      sheet.getRange(i,3).setValue(jkhasil);
      sheet.getRange(i,4).setValue(rutahasil);
      sheet.getRange(i,5).setValue(catatan);
 ...
 // Logger.log(flag);
  ...
  if(flag==1){
  var rowData = sheet.appendRow([idnks,jksebelum,jkhasil,rutahasil,catatan,new Date]);  
